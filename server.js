...
const SNAPSHOT_FILE = path.join(BACKEND_ROOT, "data", "snapshots.json");
...
const REDFOX_ENDPOINTS = {
  ...
  gzhSearchArticle: "/story/api/gzhData/searchArticle",
...
function getKeywordSearchTime(item, platform) {
  ...
  return getPublishedAt(item) || Date.now();
}

function mapKeywordSearchRawItem(item, platform, keyword, fallbackRank = 50) {
  const title = pickTitle(item);
  ...
  return {
    title,
    platform,
    rank: Number(item.rank || item.index || item.position || item.order || fallbackRank),
    interactions: pickHotValue(item, fallbackRank),
    url: pickUrl(item, platform, title),
    fetchedAt: getKeywordSearchTime(item, platform),
    summary,
    keyword,
  };
}

async function fetchRedfoxKeywordPlatformItems(platform, keyword, clientProfile) {
  const endpoint =
    platform === "抖音"
      ? REDFOX_ENDPOINTS.douyinSearchArticle
      : platform === "小红书"
        ? REDFOX_ENDPOINTS.xhsSearchArticle
        : REDFOX_ENDPOINTS.gzhSearchArticle;
  const sortType = platform === "公众号" ? "_0" : "default";
  const data = await postRedfox(endpoint, { keyword, offset: 0, sortType }, clientProfile);
  return getList(data)
    .map((item, index) => mapKeywordSearchRawItem(item, platform, keyword, index + 1))
    .filter(Boolean)
    .slice(0, 40);
}

function mergeKeywordSearchItems(rawItems, keyword, selectedPlatforms) {
  const merged = new Map();
  rawItems
    .filter((item) => item?.title)
    .forEach((item) => {
      ...
      const current = merged.get(key) || {
        title: item.title,
        industry: detectIndustry(`${item.title} ${item.summary || ""}`),
        platforms: [],
        sourceUrls: {},
        interactions: 0,
        bestRank: 99,
      ...
      if (!current.platforms.includes(item.platform)) current.platforms.push(item.platform);
      current.sourceUrls[item.platform] = item.url || platformSearchUrls[item.platform](item.title);
      current.interactions += item.interactions || 0;
      current.bestRank = Math.min(current.bestRank, item.rank || 99);
      current.latestAt = Math.max(current.latestAt, item.fetchedAt || Date.now());
      merged.set(key, current);
    });
  ...
  return [...merged.values()]
    .map((item) => {
      const coverageBase = Math.max(1, selectedPlatforms.length);
      const coverage = (item.platforms.length / coverageBase) * 100;
      const interactionScore = Math.min(100, Math.round(item.interactions / 120000));
      const freshness = Math.max(60, 100 - Math.floor((Date.now() - item.latestAt) / 3600000) * 6);
      const rise = Math.max(24, 100 - item.bestRank * 3);
      const fit = 100;
      const score = Math.round(coverage * 0.24 + interactionScore * 0.36 + rise * 0.2 + freshness * 0.2);
      return {
        ...item,
        rise,
        freshness,
        fit,
        score,
        reason: buildKeywordReason(item, keyword),
      };
    })
    .sort((a, b) => b.score - a.score);
}
...
async function searchHotItems(clientProfile, filters = {}) {
  ...
  if (!keyword) {
    return getHotItems(clientProfile);
  }
  ...
  try {
    ...
    if (request.url.startsWith("/api/hot")) {
      ...
      sendJson(
        ...
        200,
        keyword
          ? await searchHotItems(detectClientProfile(request), { keyword, platforms })
          : await getHotItems(detectClientProfile(request)),
      );
      return;
    }
